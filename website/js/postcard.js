...
function request2(form) {
    ...
    const Http = new XMLHttpRequest();
    const url = '/api/ShowCardid/' + form.cardid.value;
    Http.open("GET", url);
    Http.onload = function (e) {
...
function request4(form){
    const Http = new XMLHttpRequest();
    const url = '/api/DeletePostcard/' + form.cardid.value;
...
function request5(form){
    const Http = new XMLHttpRequest();
    const url = '/api/UpdateRating/' + form.cardid.value + "/"+ form.rating.value;
    Http.open("Put", url);
    Http.send();
    return false;
}
